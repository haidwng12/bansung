--[[
    HAI DWNG - ARSENAL ULTIMATE (Key System + HWID)
    Yêu cầu key hợp lệ từ server ngrok, mỗi key gán HWID riêng, hạn 5 giờ.
]]
...
-- ========== CẤU HÌNH KEY (THAY LINK NGrok CỦA MÀY) ==========
local API_URL = "https://agenda-cola-seismic.ngrok-free.dev/api/validate"  -- 👈 THAY LINK NÀY
...
local function getHWID()
    ...
    return tostring(game.PlaceId) .. tostring(LocalPlayer.UserId)
end

-- ========== GỌI API XÁC THỰC ==========
local function verifyKey(key, hwid)
    local http = game:GetService("HttpService")
    local body = http:JSONEncode({key = key, hwid = hwid})
    local success, response = pcall(function()
        return game:HttpGet(API_URL, true, {["Content-Type"] = "application/json"})
    end)
    if not success then return nil, "Không thể kết nối server key" end
    local data = http:JSONDecode(response)
    return data.status, data.message, data.expiry
end
...
local function showKeyUI()
    ...
    keyBox.PlaceholderText = "Nhập key (haidwng12-hour-xxxx)"
    ...
    btn.Position = UDim2.new(0.5,-70,0,125)
    btn.Text = "KÍCH HOẠT"
    ...
    status.Position = UDim2.new(0.05,0,0,185)
    status.Text = "✨ Liên hệ @haidwng12 để có key"
    ...
    timer.Size = UDim2.new(0.9,0,0,20)
    timer.Position = UDim2.new(0.05,0,0,230)
    timer.Text = ""
    timer.TextColor3 = Color3.fromRGB(100,255,100)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local hwid = getHWID()
            local stat, msg, expiry = verifyKey(key, hwid)
            if stat == "valid" then
                status.Text = "✅ " .. msg
                ...
                if expiry then
                    local function updateTimer()
                        while true do
                            ...
                            local s = remain % 60
                            timer.Text = string.format("⏳ Còn: %02d:%02d:%02d", h, m, s)
                            task.wait(1)
                        end
                    end
                ...
                task.wait(1.5)
                gui:Destroy()
                -- ===== SAU KHI KEY HỢP LỆ, CHẠY SCRIPT CHÍNH (ARSENAL) =====
                loadMainScript()
            elseif stat == "invalid" then
                status.Text = "❌ " .. msg
                status.TextColor3 = Color3.fromRGB(255,100,100)
            elseif stat == "expired" then
                status.Text = "⏰ " .. msg
                status.TextColor3 = Color3.fromRGB(255,150,50)
            else
                status.Text = "⚠️ Lỗi: " .. (msg or "không xác định")
                status.TextColor3 = Color3.fromRGB(255,100,100)
            end
        end)
    end)
...
-- ========== TOÀN BỘ SCRIPT ARSENAL (đã có từ trước) ==========
local function loadMainScript()
    -- [[ PHẦN SCRIPT ARSENAL CỦA MÀY, BẮT ĐẦU TỪ local SETTINGS ... ]]
    -- Tôi sẽ chèn chính xác nội dung script Arsenal mày đã gửi ở trên, bắt đầu từ "local SETTINGS = {" đến "print(...)"
    -- Vì quá dài, tôi đặt ở đây. (Trong thực tế, mày chỉ cần copy toàn bộ phần script Arsenal từ dòng local SETTINGS = { đến cuối, và đặt vào trong hàm này.)
    
    -- Dưới đây là phần script Arsenal gốc (đã được mày cung cấp), tôi sao chép nguyên vẹn:
    ...
    print("✅ Arsenal script đã chạy với key hợp lệ. Bật Auto Farm để treo máy.")
end

-- ========== KHỞI CHẠY: YÊU CẦU KEY TRƯỚC ==========
showKeyUI()
